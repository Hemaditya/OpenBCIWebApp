...
model = ht.NN()
model.load_state_dict(torch.load('workingHandMovement',map_location='cpu'))
model.eval()
...
def run():
	...
	while True:
		...
		for k in range(r):
			...
			strip_times = 20
			channels_times = 5
			pl = np.zeros(shape=(t*strip_times,8,45*channels_times))
			#for i in range(iterations):	
			while True:
				fftOuts = np.zeros(shape=(1,126))
				rawData[k] = ds.read_chunk().reshape(chunk_size,8)
				#fftT = np.zeros(shape=(1,126))
				for c in [5]:
					...
					no = [np.where(np.abs(notchOutput) <= 400)]
					bandpassOutput, Zstate['bandpass'][c] = bandpass(notchOutput,Zstate['bandpass'][c])
					fftO = normalize(np.abs(np.fft.rfft(notchOutput)).reshape(1,126))
					fftT = np.vstack((fftT,fftO))
				print(fftT.shape)
				fftT = fftT[1:,:40].reshape(1,-1)

				x = np.argmax(modeler.model.predict(fftT))
				if(x == 0):
					print("leg movement")
				elif(x == 1):
					print("Stable")
