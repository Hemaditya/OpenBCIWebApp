...
labels[labels == '3'] = 1
labels = labels[1:]
labels = labels.repeat(6).reshape(-1,6)
zeros = np.zeros(shape=(labels.shape[0],6),dtype='int')

labels = np.hstack((zeros,labels)).reshape(-1)
data = MNE.applyFilters(data)
data = data[12:]
X_train,X_val,y_train,y_val = train_test_split(data,labels,train_size=0.9,shuffle=True)
batch_size = 32
